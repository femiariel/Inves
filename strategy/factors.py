...
def signals_to_dataframe(signals: list[dict]) -> pd.DataFrame:
    cols = [
        ...
        "momentum_z", "trend_z", "vol_z", "drawdown_z",
        "r1M", "r3M", "r6M", "r12M",
...
def _build_raw_signal(ticker: str, history: pd.DataFrame, meta: dict) -> dict:
    ...
    momentum_raw = _weighted_momentum(r1M, r3M, r6M, r12M)
    ...
    lookback_returns = [r for r in [r1M, r3M, r6M, r12M] if not math.isnan(r)]
    data_quality_ok = (
        latest_price is not None
        and n >= MIN_OBSERVATIONS
        and _finite_all([momentum_raw, trend_raw, annual_vol, max_drawdown, max_abs_daily])
    ...
    return {
        ...
        "lookback_summary": _lookback_summary(r1M, r3M, r6M, r12M),
    }
...
def _score_signals(raw_signals: list[dict]) -> list[dict]:
    ...
    factor_values = {
        "momentum": [raw_signals[i]["momentum_raw"] for i in valid_idx],
    ...
    for i, sig in enumerate(raw_signals):
        ...
        pos = valid_pos[i]
        momentum_z = z_scores["momentum"][pos]
        ...
        sig.update({
            ...
            "risk_penalty": max(0.0, (-FACTOR_WEIGHTS["vol"] * vol_z - FACTOR_WEIGHTS["drawdown"] * drawdown_z) * 15.0),
        })
        scored.append(_finalize_signal(sig))
...
def _weighted_momentum(r1M: float, r3M: float, r6M: float, r12M: float) -> float:
    present = [
        (w, r)
        for w, r in [(W_1M, r1M), (W_3M, r3M), (W_6M, r6M), (W_12M, r12M)]
        if not math.isnan(r)
    ]
    if not present:
        return math.nan
    total_w = sum(w for w, _ in present)
    return float(sum(w * r for w, r in present) / total_w)
...
def _lookback_summary(r1M: float, r3M: float, r6M: float, r12M: float) -> str:
    def _fmt(r: float) -> str:
        return f"{r*100:+.1f}%" if not math.isnan(r) else "n/a"

    return f"1M {_fmt(r1M)} • 3M {_fmt(r3M)} • 6M {_fmt(r6M)} • 12M {_fmt(r12M)}"
